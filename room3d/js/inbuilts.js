...
demo.registerFilter('wall_box', function (box, json) {
    return {
        type: 'cube',
        width: 100,
        height: 100,
...
demo.registerFilter('racks', function (box, json) {
    ...
    if (translates) {
        for (var i = 0; i < translates.length; i++) {
            ...
            demo.copyProperties(json, rack, ['type', 'translates', 'translate', 'severities', 'rotate']);
            objects.push(rack);
        }
    }
...
demo.registerShadowPainter('rack', function (object, context, floorWidth, floorHeight, translate, rotate) {
    ...
    context.stroke();
    //旋转
    //  context.setRotation(rotate[0], rotate[1], rotate[2]);
    context.restore();
});
...
demo.registerCreator('rack', function (box, json) {
    ...
    //机柜旋转
    rack.setRotation(rotate[0], rotate[1], rotate[2]);
    ...
    rackDoor.setClient('animation.done.func', function () {
        ...
        //加载动环旋转
        fake.setRotation(rotate[0], rotate[1], rotate[2]);
        box.add(fake);
    ...
    var loader = function (box, width, height, depth, severity, rack, json) {
        ...
        //新机架选择
        newRack.setRotation(rotate[0], rotate[1], rotate[2]);
        box.add(newRack);
...
demo.registerCreator('tv', function (box, json) {
    ...
    var rotate = json.rotate || [0, 0, 0];
    var video=json.video|| demo.getRes('test.mp4');
    var parts = [{
        //tv body
        type: 'cube',
        width: 130,
    ...
    tv.setClient('dbl.func', function () {
        demo.showVideoDialog('监控大屏', video);
    });
    box.add(tv);
});

demo.registerCreator('tv1', function (box, json) {
...
demo.registerCreator('pc', function (box, json) {
    var translate = json.translate || [0, 0, 0];
    var x = translate[0],
        y = translate[1],
        z = translate[2];
    ...
    var pc = demo.createCombo(parts);
    pc.setClient('type', 'pc');
    pc.setClient('dbl.func', function () {
        demo.showVideoDialog('控制台电脑',video);
    });
    box.add(pc);
});
...
demo.registerCreator('post', function (box, json) {
    ...
    post.setClient('dbl.func', function () {
        //demo.showVideoDialog('Gleye #: C300-493A  |  Status: OK');
        demo.showPdfDialog(title,link);
    });
