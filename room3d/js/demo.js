var demo = {
    ...
    init: function (htmlElementId) {
        ...
        var gl3dview = window.network = new mono.Network3D();
        demo.typeFinder = new mono.QuickFinder(gl3dview.getDataBox(), 'type', 'client');
        demo.labelFinder = new mono.QuickFinder(network.getDataBox(), 'label', 'client');

        var gleye = new mono.PerspectiveCamera(30, 1.5, 30, 30000);
        gl3dview.setCamera(gleye);
        ...
        //this.setupControlBar(gl3dview);
        mono.Utils.autoAdjustNetworkBounds(gl3dview, document.documentElement, 'clientWidth', 'clientHeight');
        ...
        demo.setupLights(gl3dview.getDataBox());
        gl3dview.getDataBox().getAlarmBox().addDataBoxChangeListener(function (e) {
            var alarm = e.data;
            if (e.kind === 'add') {
                var node = gl3dview.getDataBox().getDataById(alarm.getElementId());
                node.setStyle('m.alarmColor', null);
            }
        });

        gl3dview.getDataBox().addDataPropertyChangeListener(function (e) {
    ...
    resetGleye: function (gl3dview) {
        gl3dview.getCamera().setPosition(2000, 1200, 3000);
        gl3dview.getCamera().lookAt(new mono.Vec3(0, 0, 0));
    },

    dirtyShadowMap: function (gl3dview) {
        var floor = gl3dview.getDataBox().shadowHost;
        var floorCombo = demo.typeFinder.findFirst('floorCombo');
        demo.updateShadowMap(floorCombo, floor, floor.getId(), gl3dview.getDataBox());
    },

    togglePersonVisible: function (visible, gl3dview) {
        var gleye = gl3dview.getCamera();
        var databox = gl3dview.getDataBox();
        if (!visible) {
    ...
    loadObj: function (gleye, box) {
        ...
        loader.load(obj, mtl, {'worker': demo.getRes('worker.png'),}, function (object) {
            ...
            var x =-550, z = 400, angle = 0;
            object.setPosition(x, 0, z);
            object.setRotationY(angle);
            var points = [[750,400],[750,260],[1150,260],[1150,-200],[850,-200],[850,230],[600,230],[600,-230],[420,-230],[420,230],[380,230],[380,-300],[220,-300],[220,230],[-550,230],];
    ...
    toggleConnectionView: function (gl3dview) {
        ...
        connections.forEach(function (connection) {
            ...
            if (!connection.billboard) {
                ...
                connection.billboard.setPosition(950,200,-100);
    ...
    loadData: function (gl3dview) {
        var json = demo.filterJson(gl3dview.getDataBox(), dataJson.objects);
        var box = gl3dview.getDataBox();
    ...
    createServer: function (box, cube, cut, pic, color, oldRack) {
        ...
        box.add(server);
        if (pic == 'equ1.png') {
            server.setClient('dbl.func', function () {
                demo.showPdfDialog("视频编码器E5710说明书", "../wiki/wiki.html#TANDBERG E5710 标清编码器");
            })
        }
        if (pic == 'equ2.png') {
            server.setClient('dbl.func', function () {
                demo.showPdfDialog("音频编码器VIBE说明书", "../wiki/wiki.html#VIBE 音频编码器");
            })
        }
        if (pic == 'equ3.png') {
            server.setClient('dbl.func', function () {
                demo.showPdfDialog("复用器MX5210说明书", "../wiki/wiki.html#TANDBERG MX5210 复用器");
            })
        }
        if (pic == 'equ4.png') {
            server.setClient('dbl.func', function () {
                demo.showPdfDialog("ASI倒换开关DEV1951说明书","../wiki/wiki.html#DEV 1951 复用器开关");
            })
        }
        if (pic == 'equ5.png') {
            server.setClient('dbl.func', function () {
                demo.showPdfDialog("调制器SM6610说明书", "../wiki/wiki.html#TANDBERG SM6610 调制器");
            })
        }
        if (pic == 'equ6.png') {
            server.setClient('dbl.func', function () {
                //demo.showPdfDialog("上变频器UT4505说明书", "pdf/equ6.pdf");
		demo.showPdfDialog("上变频器菜单", "../wiki/wiki.html#COMTECH UT4505 C波段上变频器");
            })
        }

        if (pic == 'equ7.png') {
            server.setClient('dbl.func', function () {
                demo.showPdfDialog("高功放说明书", "../wiki/wiki.html#CPI GEN IV 速调管高功放 操作培训");
        ...
        if (pic == 'equ8.png') {
            server.setClient('dbl.func', function () {
                demo.showPdfDialog("信标接收机说明书", "../wiki/wiki.html#SATCOM SINGLE TRACKING 253信标接收机");
        ...
        if (pic == 'equ9.png') {
            server.setClient('dbl.func', function () {
                demo.showPdfDialog("充气机说明书", "../wiki/wiki.html#ADH NETCOM 充气机");
        ...
        if (pic == 'equ10.png') {
            server.setClient('dbl.func', function () {
                demo.showPdfDialog("天线控制器说明书", "../wiki/wiki.html#SATCOM 7200 天线控制器");
            });
        ...
        if (pic == 'equ12.png') {
            server.setClient('dbl.func', function () {
                demo.showPdfDialog("高清调制器说明书", "../wiki/wiki.html#Newtec M6100 调制器");
        ...
        if (pic == 'equ13.png') {
            server.setClient('dbl.func', function () {
                demo.showPdfDialog("高清MPEG2编码器说明书", "../wiki/wiki.html#爱立信 AVP 2000/3000 编码器");
    ...
    createShadowImage: function (box, floorWidth, floorHeight) {
        ...
        marker(context, '控制台', '换行', 1400, 800);
        marker(context, '屏幕墙', '换行', 1400, 500);
        marker(context, '机房', '换行',2500, 900);
    ...
    toggleTemperatureView: function (gl3dview) {
        ...
        gl3dview.getDataBox().forEach(function (element) {
        ...
        if (gl3dview.temperatureView) {
            demo.createTemperatureBoard(gl3dview.getDataBox());
            demo.createTemperatureWall(gl3dview.getDataBox());
        } else {
            gl3dview.getDataBox().remove(gl3dview.getDataBox().temperaturePlane);
            delete gl3dview.getDataBox().temperaturePlane;
            gl3dview.getDataBox().remove(gl3dview.getDataBox().temperatureWall);
            delete gl3dview.getDataBox().temperatureWall;
        }
    },
            ...
            if (type === 'rack' || type === 'rack.door') {
                ...
                if (type === 'rack') {
                    if (!element.spaceCubes) {
                        element.spaceCubes = demo.createRackSpaceCubes(gl3dview.getDataBox(), element);
                    }
            ...
            if (type === 'rack' || type === 'rack.door') {
                ...
                if (type === 'rack') {
                    if (!element.usageFakeTotal) {
                        ...
                        gl3dview.getDataBox().add(usageFakeTotal);
                        ...
                        gl3dview.getDataBox().add(usageFakeUsed);
    ...
    toggleAirView: function (gl3dview) {
        ...
        if (!gl3dview.getDataBox().airPlanes) {
            gl3dview.getDataBox().airPlanes = demo.createAirPlanes();
        }

        for (var i = 0; i < gl3dview.getDataBox().airPlanes.length; i++) {
            var plane = gl3dview.getDataBox().airPlanes[i];
            if (gl3dview.airView) {
                gl3dview.getDataBox().add(plane);
                plane.airAnimation.play();
            } else {
                gl3dview.getDataBox().remove(plane);
    ...
    toggleMoveView: function (gl3dview) {
        gl3dview.getDataBox().getSelectionModel().clearSelection();
        gl3dview.moveView = !gl3dview.moveView;
        gl3dview.dirtyNetwork();
    },
    ...
    showPdfDialog: function (title, src) {
        var pdf = document.createElement('iframe');
        pdf.setAttribute('src', src);
        demo.showDialog(pdf, title, 1008, 800);
    },



    showVideoDialog: function (title,src) {
    ...
    createConnectionBillboardImage: function (value) {
        ...
        var color = 'white';
        context.font =500 + 'px "Microsoft Yahei" bold';
        context.fillStyle = color;
        context.textAlign = 'left';
        context.textBaseline = 'middle';
        context.fillText(text, height / 2, height / 5);
        ...
        context.textAlign = 'left';
        context.textBaseline = 'middle';
        context.fillText(text, height /2, height / 2);
        context.strokeStyle = color;
        context.lineWidth = 4;
    ...
    resetView: function (gl3dview) {
        ...
        for (var i = 0; i < loadedRacks.length; i++) {
            ...
            if (newRack.alarm) {
                gl3dview.getDataBox().getAlarmBox().remove(newRack.alarm);
            }
            gl3dview.getDataBox().removeByDescendant(newRack, true);

            gl3dview.getDataBox().add(oldRack);
            if (oldRack.alarm) {
                gl3dview.getDataBox().getAlarmBox().add(oldRack.alarm);
            }
    ...
    toggleWaterView: function (gl3dview) {
        ...
        if (gl3dview.waterView) {
            demo.createWaterLeaking(gl3dview.getDataBox());
            gl3dview.getDataBox().oldAlarms = gl3dview.getDataBox().getAlarmBox().toDatas();
            gl3dview.getDataBox().getAlarmBox().clear();
        } else {
            if (gl3dview.getDataBox().waterLeakingObjects) {
                for (var i = 0; i < gl3dview.getDataBox().waterLeakingObjects.length; i++) {
                    gl3dview.getDataBox().remove(gl3dview.getDataBox().waterLeakingObjects[i]);
                }
            }
            gl3dview.getDataBox().oldAlarms.forEach(function (alarm) {
                gl3dview.getDataBox().getAlarmBox().add(alarm);
            });
        }
