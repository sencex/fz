$(function () {
    ...
    proxy.query({}, '设备面板', function (result) {

        if (result && result.length > 0) {
            result.forEach(function (info) {
                tools.registerPanel(info.name, info.data);
            })
        }
        deviceEditor.refreshAccordion();

        //打开面板图
        proxy.queryFirst({name: panelName}, '设备面板', function (panelInfo) {
            var data = null;
            if (panelInfo) {
                data = panelInfo.data;
            } else {
                if (confirm('show default panel')) {
                    data = defaultPanel;
                } else {
                    data = [];//defaultPanel;
                }
    ...
    proxy.query({}, '面板部件', function (result) {

        if (result && result.length > 0) {
            result.forEach(function (info) {
                tools.registerPanelComp(info.name, info.data)
            })
        }
        deviceEditor.refreshAccordion();
    })
